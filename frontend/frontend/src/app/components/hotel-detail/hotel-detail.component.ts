...
export class HotelDetailComponent implements OnInit {
  ...
  // Local signal to cleanly isolate the manager's authorized hotel entries
  myManagedHotelsList = signal<any[]>([]);
  ...
  loadHotelDetails() {
    ...
    // Explicitly pull user ownership criteria cleanly into local memory block
    if (this.auth.isManager() && !this.auth.isAdmin()) {
      this.hotelService.getMyHotels().subscribe((res: any) => {
        const list = res?.data || res || [];
        this.myManagedHotelsList.set(list);
      });
    }
  ...
  canManage(): boolean {
    ...
    // 1. Admins have absolute override access
    if (this.auth.isAdmin()) return true;
    
    // 2. Managers can manage if this page's hotelId is inside their verified ownership array
    if (this.auth.isManager()) {
      const managedList = this.myManagedHotelsList();
      return managedList.some(h => Number(h.hotelId || h.id) === Number(this.hotelId));
    }
